...
const WriteStoryModule: React.FC<WriteStoryModuleProps> = ({ moduleState, setModuleState, retrievedViralOutlineFromAnalysis }) => {
  ...
  const handleGenerateHooks = async () => {
    // Trừ credit trước khi tạo hooks
    const hasCredits = await consumeCredit(1);
    if (!hasCredits) {
      updateState({ hookError: 'Không đủ credit để tạo hooks!' });
      return;
    }
    ...
    try {
      const result = await generateTextViaBackend({
        prompt,
        provider: apiSettings.provider as 'gemini' | 'openai' | 'deepseek',
  ...
  const handleWriteStory = async () => {
    ...
    // Trừ credit trước khi viết truyện (tính theo số phần sẽ viết)
    ...
    const totalCost = numChunks + 1; // +1 cho phần biên tập

    const hasCredits = await consumeCredit(totalCost);
    if (!hasCredits) {
      updateState({ storyError: `Không đủ credit để viết truyện! Cần ${totalCost} credit.` });
      return;
    }
  ...
  const handleEditStory = async (
    ...
    if (!externalAbortController) { // If called directly, manage its own controller
        ...
        // Trừ credit khi gọi trực tiếp (không phải từ handleWriteStory)
        const hasCredits = await consumeCredit(1);
        if (!hasCredits) {
          updateState({ storyError: 'Không đủ credit để biên tập truyện!', singleStoryEditProgress: null, storyLoadingMessage: null, hasSingleStoryBeenEditedSuccessfully: false });
          setCurrentAbortController(null);
          return;
        }
    }
  ...
  const handleGenerateLesson = async () => {
    ...
    // Trừ credit trước khi tạo bài học
    const hasCredits = await consumeCredit(1);
    if (!hasCredits) {
      updateState({ lessonError: 'Không đủ credit để tạo bài học!' });
      return;
    }
