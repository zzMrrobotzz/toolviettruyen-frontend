    ...
    <ModuleContainer title="📚 Viết Truyện Hàng Loạt">
      ...
      <div className="space-y-6 p-6 border-2 border-gray-200 rounded-lg bg-gray-50 shadow mb-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div>
            <label htmlFor="bsOutputLanguage" className="block text-sm font-medium text-gray-700 mb-1">Ngôn ngữ đầu ra:</label>
        ...
        <div>
            <label htmlFor="bsReferenceViralStory" className="block text-sm font-medium text-gray-700 mb-1">Truyện Viral Tham Khảo (học văn phong - Không bắt buộc):</label>
            <textarea id="bsReferenceViralStory" value={referenceViralStoryForStyle} onChange={(e) => updateState({ referenceViralStoryForStyle: e.target.value })} rows={3} className="w-full p-3 border-2 border-gray-300 rounded-lg shadow-sm" placeholder="Dán truyện mẫu..." disabled={isProcessingBatch}></textarea>
        </div>
      </div>

      {/* Input Items */}
      <div className="space-y-4 mb-8">
        <h3 className="text-xl font-semibold text-gray-800 mb-2">Danh sách Dàn Ý Truyện</h3>
        {inputItems.map((item, index) => (
          <div key={item.id} className="p-4 border-2 border-gray-200 rounded-lg bg-white shadow-sm space-y-3">
            <div className="flex justify-between items-center">
                ...
                <button onClick={() => handleRemoveItem(item.id)} className="text-red-500 hover:text-red-700 font-medium text-sm p-2 rounded-md hover:bg-red-50" disabled={isProcessingBatch}>
                    🗑️ Xóa
                </button>
            </div>
            <div>
              <label htmlFor={`outline-${item.id}`} className="block text-sm font-medium text-gray-700 mb-1">Dàn Ý Truyện (*):</label>
              <textarea id={`outline-${item.id}`} value={item.outline} onChange={(e) => handleInputChange(item.id, 'outline', e.target.value)} rows={5} className="w-full p-2 border border-gray-300 rounded-md shadow-sm" placeholder="Dán dàn ý đầy đủ vào đây..." disabled={isProcessingBatch}></textarea>
            </div>
            <details className="text-sm">
                ...
                <div className="mt-2 space-y-3 p-3 bg-gray-50 rounded-md border">
                    <div>
                        <label htmlFor={`specificLength-${item.id}`} className="block text-xs font-medium text-gray-600 mb-0.5">Độ dài riêng (để trống nếu dùng chung):</label>
                    ...
                    {item.specificWritingStyle === 'custom' && (
                        <div>
                        ...
                        <input type="text" id={`specificCustomStyle-${item.id}`} value={item.specificCustomWritingStyle || ""} onChange={(e) => handleInputChange(item.id, 'specificCustomWritingStyle', e.target.value)} className="w-full p-2 border border-gray-300 rounded-md text-xs" placeholder="Mô tả phong cách riêng..." disabled={isProcessingBatch}/>
                        </div>
                    )}
                </div>
            </details>
      ...
      {results.length > 0 && (
        <div className="mt-8 space-y-6">
          <h3 className="text-2xl font-semibold text-gray-800 border-b pb-2">Kết Quả Viết Hàng Loạt</h3>
          {results.map((result, index) => (
            <details key={result.id} className={`p-4 border-l-4 rounded-lg shadow-md bg-white 
              ...
              <summary className="font-semibold text-lg text-gray-700 cursor-pointer flex justify-between items-center">
                <span>Truyện #{index + 1}: {result.originalOutline.substring(0, 60)}...</span>
              ...
              <div className="mt-4 space-y-4">
                ...
                {result.generatedStory && (
                    <div>
                        <h5 className="text-md font-semibold text-gray-600 mb-1">Truyện Đã Tạo & Biên Tập:</h5>
                ...
                {result.postEditAnalysis && (
                    <div className="mt-4 p-3 border border-teal-300 rounded-md bg-teal-50">
                        <h5 className="text-md font-semibold text-teal-700 mb-2">Báo Cáo Phân Tích Sau Biên Tập:</h5>
                        ...
                        <ul className="list-disc list-inside ml-4 text-xs text-gray-700">
                            {result.postEditAnalysis.keyImprovements.map((imp, i) => <li key={i}>{imp}</li>)}
                        </ul>
                        <p className="text-sm text-teal-600 mt-1"><strong>Vấn đề còn lại (nếu có):</strong></p>
                         {result.postEditAnalysis.remainingIssues.length > 0 ? (
                            <ul className="list-disc list-inside ml-4 text-xs text-gray-700">
                                {result.postEditAnalysis.remainingIssues.map((iss, i) => <li key={i}>{iss}</li>)}
                            </ul>
                         ) : <p className="text-xs text-gray-600 italic">Không có vấn đề đáng kể.</p>}
                    </div>
                )}
                 <div>
                    <h5 className="text-xs font-semibold text-gray-500 mb-0.5 uppercase">Dàn Ý Gốc:</h5>
                    <textarea value={result.originalOutline} readOnly rows={3} className="w-full p-1 border border-gray-200 rounded-md bg-gray-100 text-xs whitespace-pre-wrap" disabled></textarea>
                </div>
              </div>
            </details>
          ))}
