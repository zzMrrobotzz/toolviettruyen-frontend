...
const SuperAgentModule: React.FC<SuperAgentModuleProps> = ({ 
  ...
  const generateImage = async (prompt: string, aspectRatio: string = "16:9") => {
    const result = await generateImageViaBackend(prompt, aspectRatio, 'gemini');
    if (!result.success) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (let i = 0; i < prompts.length; i++) {
        ...
        const imageB64 = await generateImage(prompts[i], aspectRatio);
        if (abortController.signal.aborted) throw new DOMException('Aborted', 'AbortError');
