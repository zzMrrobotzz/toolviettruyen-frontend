...
export const fetchPackages = async () => {
    try {
        const response = await retryRequest(() => apiClient.get('/packages'));
        
        // Validate response is array
        const data = response.data;
        if (!Array.isArray(data)) {
          console.warn('Packages response is not array:', data);
          return [];
        }
...
export const updatePackage = async (packageId: string, payload: any) => {
    try {
        if (!packageId || typeof packageId !== 'string') {
          throw new Error('ID gói cước không hợp lệ.');
        }
...
export const createPackage = async (payload: any) => {
    try {
        if (!payload || typeof payload !== 'object') {
          throw new Error('Dữ liệu gói cước không hợp lệ.');
        }
